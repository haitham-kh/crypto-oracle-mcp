"""
train_v5.py - Final all-out training pipeline.

Same multi-horizon (60 / 240 / 720) classifier+regressor architecture as v4,
but trained on the new feature stack:

    62  v4 microstructure / regime / vol-profile features
    12  v5 perp features (funding, OI, LSR, taker ratio)
     6  v5 cross-sectional features (ranks vs basket, alpha vs BTC)
    14  one-hot coin identity (per-coin specialization without separate models)
    ─
    94  total

The classifier asks: P(fee-positive trade at horizon h | features).
The regressor asks: E[forward log-return at horizon h | features].

Per-horizon EV-tuned thresholds, isotonic calibration on validation, true
out-of-sample test slice. Same temporal walk-forward 70/15/15 split as v4.
...
Outputs (overwrite previous models for a clean slate):
    data/v5_clf_h{60,240,720}.json
    data/v5_reg_h{60,240,720}.json
    data/v5_calib_h{60,240,720}.pkl
    data/ev_model_v5_meta.json
"""
...
from features_v5 import (FEATURE_NAMES_V5, FEATURE_NAMES_V5_EXTRA, build_v5_full)
...
META_PATH = os.path.join(DATA_DIR, "ev_model_v5_meta.json")
...
def _coin_onehot(symbol, n):
    """Return (n, 14) one-hot column block for this coin."""
...
def process_coin(symbol, btc_ts, btc_close, basket_state):
    ...
    print(f"    [{symbol}] v2 features...", flush=True)
    F_v2, atr_arr = build_features_v2(close, high, low, vol, ofi_raw, ts_ms, btc_aligned)
    print(f"    [{symbol}] v2 done ({time.time()-t0:.1f}s); v5 stack...", flush=True)
    t1 = time.time()
    X_features = build_v5_full(symbol, F_v2, close, high, low, vol, ofi_raw,
                               tbv, ts_ms, atr_arr, btc_aligned, PERP_DIR,
                               basket_state)
    onehot = _coin_onehot(symbol, n)
    X = np.hstack([X_features, onehot])
    print(f"    [{symbol}] v5 features done ({time.time()-t1:.1f}s, "
...
# ── XGB params (slightly more capacity for the bigger feature stack) ───────
...
def train_horizon(h, X_train, ts_train, y_train, X_val, y_val, net_val,
    ...
    clf = xgb.train(XGB_CLF_PARAMS, dtr, num_boost_round=800,
    ...
    if m_t.sum() > 0:
        ...
        wins_t = nets_t[nets_t > 0].sum(); losses_t = -nets_t[nets_t <= 0].sum()
    ...
    rm_tr = ~np.isnan(ret_train); rm_va = ~np.isnan(ret_val)
    ...
    reg = xgb.train(XGB_REG_PARAMS, dtr_r, num_boost_round=600,
    ...
    # Top feature importances (gain) — interpretability sanity check.
    try:
        imp = clf.get_score(importance_type="gain")
        ranked = sorted(imp.items(), key=lambda x: -x[1])[:12]
        print("    top-12 classifier features (gain):")
        for name, g in ranked:
            print(f"      {name:<28} {g:>10.2f}")
    except Exception:
        pass

    clf_path = os.path.join(DATA_DIR, f"v5_clf_h{h}.json")
    reg_path = os.path.join(DATA_DIR, f"v5_reg_h{h}.json")
    cal_path = os.path.join(DATA_DIR, f"v5_calib_h{h}.pkl")
...
def main():
    t0 = time.time()
    print("=" * 78)
    print("  EV Model V5 — multi-horizon + perp microstructure + cross-sectional")
    print(f"  Horizons: {HORIZONS}   round-trip cost: {ROUND_TRIP_COST*100:.2f}%")
    print(f"  Features: {len(ALL_FEATURE_NAMES)} "
          f"(v4=62 + perp=12 + xs=6 + coin-onehot=14)")
    print("=" * 78)
    ...
    if os.path.exists(BASKET_FP):
        print(f"Loading basket state from {BASKET_FP}...")
        basket_state = pl.read_parquet(BASKET_FP)
        print(f"  basket rows: {basket_state.height:,}")
    else:
        print("[!] basket state not found; cross-sectional features will be zeros.")
        print("    Run `python basket_alignment.py` first for the full pipeline.")

    if not os.path.isdir(PERP_DIR) or not glob.glob(os.path.join(PERP_DIR, "*.parquet")):
        print("[!] perp data not found; perp features will be zeros.")
        print("    Run `python download_perp_data.py` first for the full pipeline.")
    ...
    bundles = []
    for sym in symbols:
        b = process_coin(sym, btc_ts, btc_close, basket_state)
        if b is not None:
            bundles.append(b)
    ...
    meta = {
        "model_type": "xgboost_v5_perp_xs_coin_onehot",
